...
let orderByindex = -1;
	...
	function deleteOrder(){
		DataOrderStatus.DeleteOrder(detailOrder)
			.then((response) => {
				handleMessageShow();
					...
					<div className="card mb-4 card-div ">
						<div className="col-md-12 row">
							...
							ID:{item.id}
								...
								<button className="btn_order_status_cancel outfix" type="button" onClick={deleteOrder}>Hủy Đơn Hàng</button>
							</div>
						</div>
					</div>
				)
...
<Modal dialogClassName="messmodal" show={showmes} onHide={handleMessageClose}>
			...
			<i class="fa fa-check" aria-hidden="true"></i>
				...
				<Modal.Body>
					<p className="text-center mt-4">Hủy Thành Công</p>
				</Modal.Body>
